(async function () {
  const STATUS_KEY = 'alo-planner-status-v1';
  const OVERRIDE_KEY = 'alo-planner-overrides-v1';
  const CUSTOM_KEY = 'alo-custom-posts-v1';
  const ORDER_KEY = 'alo-order-v1';
  const NOTES_KEY = 'alo-notes-v1';
  const CAPTION_KEY = 'alo-captions-v1';
  const FORMATS = [
  ...
  function downloadMedia(ref, filename) {
    ...
    a.download = filename || (String(ref).startsWith('data:') ? 'alo.jpg' : String(ref).split('/').pop());
  ...
  /* Fetch image listings: python http.server directory listing locally,
     manifest.json on static hosts without listings (GitHub Pages). */
    ...
    try {
      ...
      if (r.ok) {
        ...
        const matches = [...html.matchAll(/href="([^"?]+)"/g)].map(m => m[1])
          .filter(f => /\.(jpe?g|png|gif|webp|mp4|mov|webm)$/i.test(f));
        if (matches.length) return matches;
      }
    } catch {}
    try {
      const m = await fetch(path + 'manifest.json', { cache: 'no-store' });
      if (m.ok) { const j = await m.json(); if (Array.isArray(j)) return j; }
    } catch {}
    return [];
  }
  ...
  function bindBio() {
    document.querySelector('.bio').innerHTML =
      'Alo Yoga в Україні 🇺🇦<br>' +
      'Оригінал зі США 🇺🇸 · в наявності та під замовлення<br>' +
      'Доставка Новою поштою по Україні<br>' +
      'Замовлення — у Direct';
  }
  ...
  function renderFilters() {
    ...
    if (prodChip) {
      ...
      prodChip.textContent = (state.productionOnly ? '✓ ' : '') + `Media to source (${count})`;
    }
  }
  ...
  function renderStats() {
    ...
    add(sts, 'stat', `<div class="num">${posts.filter(p => p.productionRequired).length}</div><div class="lbl">to source</div>`);
  ...
  function openModal(p) {
    ...
    if (p.format === 'carousel' && p.slides) {
      ...
      lg.appendChild(el('div', 'lab', 'Media to source'));
    ...
    if (!getThumbnail(p) && p.format !== 'carousel' && p.format !== 'stories-only') {
      const addBtn = el('button', 'slide-add-btn', '+ Add photo / video');
      addBtn.style.margin = '12px';
      addBtn.onclick = (e) => { e.stopPropagation(); openPicker(file => { setImageOverride(p.id, file); openModal(p); renderGrid(); }); };
    ...
    if (p.productionRequired) {
      const prb = badge('', 'SOURCE · ' + (p.productionType || '').replace('-', ' '));
    ...
    nSec.appendChild(elInner('h4', '', 'Team note'));
    const noteInput = el('textarea', 'note-input');
    noteInput.value = getNote(p.id);
    noteInput.placeholder = 'Leave a note for the team…';
    ...
    if (p.productionRequired && p.productionBrief) {
      ...
      pb.appendChild(elInner('h4', '', 'Media brief (що зберегти)'));
      pb.appendChild(el('div', 'brief', p.productionBrief));
      c.appendChild(pb);
    }

    // ChatGPT brief — the task the AI copywriter gets for this post
    if (p.aiBrief) {
      const ab = el('div', 'modal-section');
      ab.appendChild(elInner('h4', '', 'ChatGPT brief (завдання)'));
      ab.appendChild(el('div', 'brief', p.aiBrief));
      c.appendChild(ab);
    }
    ...
    // Caption (editable)
    {
      ...
      if (state.editingCaption === p.id) {
        ...
        if (state.aiDraft && state.aiDraft.id === p.id) { ta.value = state.aiDraft.text; state.aiDraft = null; }
        ...
        btns.appendChild(mkBtn('✨ Regenerate', (e) => generateCaptionFor(p, e.currentTarget), true));
        ...
        btns.appendChild(mkBtn('✨ Generate (ChatGPT)', (e) => generateCaptionFor(p, e.currentTarget), true));
        if (capText) {
  ...
  function exportData() {
    const dump = { _app: 'alo-ukraine-planner', _version: 1, data: {} };
    ALL_KEYS.forEach(k => { const v = localStorage.getItem(k); if (v != null) dump.data[k] = v; });
    const blob = new Blob([JSON.stringify(dump, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = 'alo-ukraine-planner-data.json';
  ...
  const CLIENT_ID = (function () {
    let id = localStorage.getItem('alo-client-id');
    if (!id) { id = Math.random().toString(36).slice(2) + Date.now().toString(36); try { localStorage.setItem('alo-client-id', id); } catch {} }
    return id;
  })();
  ...
  async function pushRemote() {
    ...
    const payload = { _app: 'alo-ukraine', _updatedAt: Date.now(), _client: CLIENT_ID, data: buildOverlay() };
  ...
  /* ---------- ChatGPT (OpenAI API) caption generator ---------- */
  const AI_KEY = 'alo-ai-openai-key';
  const AI_MODEL_KEY = 'alo-ai-model';
  const AI_STYLE_KEY = 'alo-ai-style';
  const AI_DEFAULT_MODEL = 'gpt-5-mini';
  function aiKey() { return localStorage.getItem(AI_KEY) || ''; }
  function aiModel() { return localStorage.getItem(AI_MODEL_KEY) || AI_DEFAULT_MODEL; }
  function aiStyle() {
    return localStorage.getItem(AI_STYLE_KEY)
      || state.data?.meta?.aiStylePrompt
      || 'Пиши українською. Коротко, впевнено, без канцеляриту. 2–4 короткі речення, в кінці — заклик написати в Direct.';
  }
  ...
  function captionMessages(info, current) {
    const meta = state.data?.meta || {};
    const sys = 'You are the SMM copywriter for ' + (meta.brand || 'ALO UKRAINE') +
      ' (' + (meta.handle || '@alo.ukraine') + '), a Ukrainian Instagram shop reselling original Alo Yoga.' +
      '\nWrite ONE Instagram caption in Ukrainian following this style guide:\n' + aiStyle() +
      '\nRules: return ONLY the caption text — no hashtags, no surrounding quotes, no explanations.';
    let user = 'Пост:\n— Назва: ' + (info.title || '—') +
      '\n— Формат: ' + (info.format || 'static') +
      '\n— Рубрика: ' + (info.pillar || '—');
    if (info.product) user += '\n— Товар: ' + info.product;
    if (info.priceUAH) user += '\n— Ціна: ' + info.priceUAH + ' ₴';
    if (info.task) user += '\n— Завдання від бренд-менеджера (виконати повністю): ' + info.task;
    if (info.brief) user += '\n— Що на фото/відео: ' + info.brief;
    if (current) user += '\n\nПоточна чернетка (перепиши краще, збережи всі факти й ціни):\n' + current;
    user += '\n\nНапиши підпис.';
    return [{ role: 'system', content: sys }, { role: 'user', content: user }];
  }
  function postAiInfo(p) {
    return {
      title: p.title,
      format: p.format,
      pillar: state.data.pillars[p.pillar]?.label || p.pillar,
      product: p.product,
      priceUAH: p.priceUAH,
      task: p.aiBrief,
      brief: p.productionBrief || p.imageBrief,
    };
  }
  async function generateCaptionFor(p, btn) {
    if (!aiKey()) { toast('Add your OpenAI API key first'); openAiSettings(); return; }
    const old = btn.textContent;
    btn.textContent = '⏳ Generating…'; btn.disabled = true;
    try {
      const text = await callChatGPT(captionMessages(postAiInfo(p), getEffectiveCaption(p)));
      state.aiDraft = { id: p.id, text };
      state.editingCaption = p.id;
      openModal(p);
      toast('Draft generated — review and Save');
    } catch (err) {
      btn.textContent = old; btn.disabled = false;
      alert('ChatGPT error: ' + err.message);
    }
  }
  // Batch: ChatGPT writes captions for every post (only empty ones by default).
  async function generateAllCaptions(btn) {
    if (!aiKey()) { toast('Add your OpenAI API key first'); openAiSettings(); return; }
    const posts = allPosts();
    const empty = posts.filter(p => !(getEffectiveCaption(p) || '').trim());
    let targets = null;
    if (empty.length) {
      if (confirm('ChatGPT напише підписи для ' + empty.length + ' постів без тексту.\n\nOK — почати · Cancel — скасувати')) targets = empty;
    } else {
      if (confirm('Усі пости вже мають підписи. Переписати ВСІ ' + posts.length + ' постів через ChatGPT?')) targets = posts;
    }
    ...
    for (const p of targets) {
      ...
      try {
        ...
        if (/api key|incorrect|401/i.test(err.message)) break; // no point retrying every post with a bad key
      }
      done++;
    }
    btn.textContent = old; btn.disabled = false;
    renderAll();
    if (errors.length) alert('Готово з помилками (' + errors.length + '):\n\n' + errors.slice(0, 5).join('\n'));
    else toast('ChatGPT написав ' + done + ' підписів — перегляньте пости');
  }

  function openAiSettings() {
    const body = $('#aiModalBody'); body.innerHTML = '';
    body.appendChild(el('h2', 'editor-title', '✨ ChatGPT settings'));
    function field(labelText, node, hint) {
      const f = el('div', 'ed-field');
      f.appendChild(el('label', 'ed-label', labelText));
      f.appendChild(node);
      if (hint) f.appendChild(el('div', 'ed-hint', hint));
      return f;
    }
    const keyIn = Object.assign(el('input', 'ed-input'), { type: 'password', value: aiKey(), placeholder: 'sk-…' });
    const modelIn = Object.assign(el('input', 'ed-input'), { value: aiModel(), placeholder: AI_DEFAULT_MODEL });
    const styleIn = Object.assign(el('textarea', 'ed-input ed-textarea'), { value: aiStyle() });
    styleIn.style.minHeight = '180px';
    body.appendChild(field('OpenAI API key', keyIn, 'Create at platform.openai.com → API keys. Stored only in this browser (LocalStorage), never uploaded anywhere else.'));
    body.appendChild(field('Model', modelIn, 'e.g. gpt-5-mini (fast & cheap), gpt-5, gpt-4o.'));
    body.appendChild(field('Style guide — how ChatGPT writes your captions', styleIn, 'Edit freely: tone, length, emoji, CTA, examples of captions you like.'));
    const actions = el('div', 'ed-actions');
    actions.appendChild(mkBtn('Save', () => {
      try {
        localStorage.setItem(AI_KEY, keyIn.value.trim());
        localStorage.setItem(AI_MODEL_KEY, modelIn.value.trim() || AI_DEFAULT_MODEL);
        localStorage.setItem(AI_STYLE_KEY, styleIn.value.trim());
      } catch {}
      closeAiSettings(); toast('AI settings saved');
    }));
    actions.appendChild(mkBtn('Reset style to default', () => {
      try { localStorage.removeItem(AI_STYLE_KEY); } catch {}
      styleIn.value = aiStyle();
    }, true));
    actions.appendChild(mkBtn('Cancel', closeAiSettings, true));
    body.appendChild(actions);
    $('#aiModal').classList.add('open');
  }
  function closeAiSettings() { document.getElementById('aiModal').classList.remove('open'); }
  function bindAI() {
    const btn = $('#aiSettingsBtn');
    if (btn) btn.onclick = openAiSettings;
    const genAll = $('#generateAllBtn');
    if (genAll) genAll.onclick = (e) => generateAllCaptions(e.currentTarget);
    const close = document.getElementById('aiModalClose');
    if (close) close.onclick = (e) => { e.stopPropagation(); closeAiSettings(); };
    const backdrop = document.querySelector('#aiModal .modal-backdrop');
    if (backdrop) backdrop.onclick = closeAiSettings;
    const modal = document.getElementById('aiModal');
    if (modal) modal.addEventListener('click', (e) => { if (e.target.id === 'aiModal') closeAiSettings(); });
  }
  ...
  function openEditor(existing) {
    ...
    const titleIn = Object.assign(el('input', 'ed-input'), { value: editing ? (existing.title || '') : '', placeholder: 'e.g. Airlift Legging — new drop' });
    ...
    const briefIn = Object.assign(el('textarea', 'ed-input ed-textarea'), { value: editing ? (existing.aiBrief || '') : '', placeholder: 'Завдання для ChatGPT: що сказати, товар, ціна, наявність, CTA…' });
    const capIn = Object.assign(el('textarea', 'ed-input ed-textarea'), { value: editing ? (existing.caption || '') : '', placeholder: 'Caption text…' });
    const hashIn = Object.assign(el('input', 'ed-input'), { value: editing && existing.hashtags ? existing.hashtags.join(' ') : '', placeholder: '#aloyoga #aloukraine (max 5)' });
    ...
    body.appendChild(field('ChatGPT brief (завдання для копірайтера)', briefIn));
    const capField = field('Caption', capIn);
    const aiRow = el('div', 'btn-row');
    aiRow.appendChild(mkBtn('✨ Generate with ChatGPT', (e) => {
      const btn = e.currentTarget;
      if (!aiKey()) { toast('Add your OpenAI API key first'); openAiSettings(); return; }
      const info = {
        title: titleIn.value.trim(),
        format: fmtSel.value,
        pillar: state.data.pillars[pillarSel.value]?.label || pillarSel.value,
        task: briefIn.value.trim(),
      };
      const old = btn.textContent;
      btn.textContent = '⏳ Generating…'; btn.disabled = true;
      callChatGPT(captionMessages(info, capIn.value))
        .then(text => { capIn.value = text; })
        .catch(err => alert('ChatGPT error: ' + err.message))
        .finally(() => { btn.textContent = old; btn.disabled = false; });
    }, true));
    capField.appendChild(aiRow);
